...
public:
    ...
    virtual void end() = 0;
    virtual void save_progress(const string &filename) const = 0;
    virtual void load_progress(const string &filename) = 0;
...
public:
    ...
    void start() override {
        while (!is_won() && !is_lost()) {
            ...
            cin >> guess;
            guess = tolower(guess); // Ensure the letter is lowercase
